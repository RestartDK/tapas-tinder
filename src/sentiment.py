...
def save_checkpoint(processed_ids, sentiment_results):
    ...
    results_checkpoint = os.path.join(
        PROJECT_ROOT, "data", "sentiment_results_checkpoint.json"
    )
    results_to_save = []
...
def analyze_aspect_with_word2vec(
    tokens, word2vec_model, aspect_vectors, similarity_threshold=0.4
):
    """
    ...
    try:
        for i, review in enumerate(reviews):
            ...
            try:
                ...
                aspect_scores = {"food": [], "service": [], "value": [], "ambiance": []}
                ...
                for sentence in sentences:
                    ...
                    aspect_confidences = analyze_aspect_with_word2vec(
                        tokens, word2vec_model, aspect_vectors
                    )
                ...
                if overall_score >= 0.5:
                    emotions.append(
                        "joy" if overall_magnitude > 0.5 else "satisfaction"
                    )
                elif overall_score >= 0.1:
                    emotions.append("contentment")
                elif overall_score <= -0.5:
                    emotions.append(
                        "anger" if overall_magnitude > 0.5 else "frustration"
                    )
                elif overall_score <= -0.1:
